...
var apiSecenekleri = {
    ...
    //sunucu:"https://mekanbul.yusufdd.repl.co",
    apiYolu: "/api/mekanlar/",
    adminApiYolu: "/api/admin/mekanlar",
...
const adminKayitOl = function (req, res) {
    res.render('kayitol', { "title": "Admin Page" });
}
const adminKayitEt = function (req, res) {
    var adminKayitBilgileri;
    adminKayitBilgileri = {
        adsoyad: req.body.adsoyad,
        eposta: req.body.eposta,
        sifre: req.body.sifre
    }
    axios.post(apiSecenekleri.sunucu + apiSecenekleri.adminKayitApiYolu, adminKayitBilgileri).then(function () {
        res.redirect("/admin")
    });
}
const adminSayfa = function (req, res) {
    axios.get(apiSecenekleri.sunucu + apiSecenekleri.adminApiYolu).then(function (response) { //tüm mekanları dolaştık
        var mekanlar;
        mekanlar = response.data; //mekanlara ulaşmamızı sağlar
        adminSayfaOlustur(res,req,mekanlar);
    }).catch(function (hata) {
        adminSayfaOlustur(res,req, hata);
    });
}
...
const adminMekanEkle = function (req, res) {
    ...
    if (!req.body.mekanAdi || !req.body.mekanAdresi || !req.body.imkanlar || !req.body.enlem ||
        ...
        axios.post(apiSecenekleri.sunucu + apiSecenekleri.apiYolu, eklenenMekanBilgileri).then(function () {
            res.redirect("/admin/adminsayfa")
        });
    };
}
